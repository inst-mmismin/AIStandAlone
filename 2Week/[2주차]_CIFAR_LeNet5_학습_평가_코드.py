...
class myLeNet5_Sequential(nn.Module): 
    def __init__(self, num_classes): 
        super().__init__()
...
class myLeNet5_ModuleList(nn.Module): 
    def __init__(self, num_classes): 
        super().__init__()
    ...
    def forward(self, x):
        batch_size = x.shape[0]
        for module in self.front:
            x = module(x)
        x = x.reshape(batch_size, -1)
        for module in self.back:
            x = module(x)
        return x
...
model = myLeNet5_base(num_classes).to(device)
